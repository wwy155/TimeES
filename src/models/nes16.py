...
class NeuralEvolutionarySpectra(nn.Module):
    def __init__(
        ...
        self.omegas =  torch.fft.fftfreq(input_len)
        # Register buffers
        ...
        self.A_scale_predictor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, 2 * (input_len + out_len) * self.fft_len)
        )
    ...
    def forward(self, X, t_index_in, t_index_out):
        ...
        A_scale = self.A_scale_predictor(X).view(B, self.input_len + self.out_len, self.fft_len, 2)
        if self.additive_scale:
            A_half = torch.view_as_complex(A_scale + STFT_init.unsqueeze(1))
        else:
            A_half = torch.view_as_complex(A_scale * STFT_init.unsqueeze(1))
        A_all = construct_hermitian_spectrum(A_half, self.input_len)
