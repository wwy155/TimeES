...
class NeuralEvolutionarySpectra(nn.Module):
    def __init__(
        ...
        self.A_predictor = nn.Sequential(
            ...
            nn.Linear(input_len + 2  * topk + topk, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, 2 * (input_len + out_len) * topk)
        )
        
        self.predict_omegas = predict_omegas
        if predict_omegas:
            self.omegas_predictor = nn.Sequential(
                nn.Linear(topk, hidden_dim),
                nn.ReLU(),
                nn.Linear(hidden_dim, topk*(input_len + out_len))
            )
    ...
    def forward(self, X, t_index_in, t_index_out):
        ...
        zero_freq_all = self.zero_frequencies(X)
        ...
        A_all = torch.view_as_complex(self.A_predictor(predict_inp).view(B, self.input_len + self.out_len, self.topk, 2))  # [B, 2 *( input_len + out_len )* K_eff]
        if self.predict_omegas:
            all_rec = zero_freq_all + synthesize_signal_piece_omegeas(A_all, selected_omegas, self.input_len, torch.arange(0, self.input_len+self.out_len).to(device))  # [B, input_len]
        else:
            all_rec = zero_freq_all + synthesize_signal_on_subband(A_all, selected_omegas, self.input_len, torch.arange(0, self.input_len+self.out_len).to(device))  # [B, input_len]
