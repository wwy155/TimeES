...
class NeuralEvolutionarySpectra(nn.Module):
    ...
    def forward(self, X, t_index_in, t_index_out):
        ...
        STFT_init_complex = self.A_init[t_index_stft][:,  self.freq_indices] # B, M//2 + 1 clofat
        STFT_init = torch.view_as_real(STFT_init_complex) # B, M//2 + 1, 2

        A_scale = self.A_scale_predictor(X).view(B, self.input_len + self.out_len, self.K, 2)
