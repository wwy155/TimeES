...
from src.models.nesr3 import NeuralEvolutionarySpectra
...
class NESReconstruct(ReconstructExp, NESParameters):
    ...
    def _init_model(self):
        ...
        self.model = NeuralEvolutionarySpectra(
            ...
            self.M,
            self.device,
            torch.complex(torch.tensor(np.sqrt(S_init)).float(), torch.tensor(np.zeros_like(S_init)).float()),
            omega
        )
