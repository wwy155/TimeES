...
class Parameters:
    ...
    backbone : str = 'linear' # PatchTST iTransformer

@dataclass
class TimeESForecast(ForecastExp, Parameters):
    model_type: str = "TimeES"

    def _init_model(self):
        for energy_ratio in [0.1, 0.2, 0.4,  0.6, 0.8, 0.9, 0.95]:
            scaled_data = self.scaler.transform(self.dataset.data)[:, 0:2]
            print(scaled_data.shape)
            A_train_init, _  = get_initial_amplitude_right_onesided_mv(
                torch.tensor(self.dataloader.train_dataset.scaled_data[:, 0:2]).transpose(0, 1), 
                n_fft=self.M, 
            ) 
            # A_train_init N, T, M
            ...
            if energy_ratio == 1:
                ...
                selected_freqs = select_frequencies_by_energy_ratio_batch(energy_per_frame, energy_ratio)
            print(f"r={energy_ratio} K={len(selected_freqs[0])} selected_freqs={selected_freqs}")
    ...
    def _test(self):
        self.plot()
        return super(TimeESForecast, self)._test()
