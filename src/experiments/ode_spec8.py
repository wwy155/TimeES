...
class ODESpecExp(ForecastExp, WFParameters):
    ...
    def _init_model(self):
        ...
        self.freq_dim = (self.fft_length//2 + 1)
    ...
    def compute_stft_spectrum(self, x):
        ...
        stft_out = torch.stft(
            ...
            onesided=True,
            center=False
        )  # [B*N, n_fft//2+1, 1 + (L - fft_length) // hop_length]
        
        stft_out = stft_out.permute(0, 2, 1).reshape(B, N, 1 + (L - self.fft_length) // self.hop_length, (self.fft_length//2+1))  # [B, num of freq_spectrum, K//2*2+2]
        return stft_out
