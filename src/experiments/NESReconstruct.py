...
from src.experiments.NES import NESForecast
...
@dataclass
class NESReconstruct(NESForecast):
    model_type: str = "NESReconstruct"
    horizon:int  = 1
    pred_len:int  = 1
    ...
    def _process_train_batch(self, batch_x, batch_y, origin_x, origin_y, batch_x_date_enc, batch_y_date_enc, x_index, y_index):
        ...
        return results[:, -self.windows:, :], batch_y, A[:, -1, :, :]
    ...
    def _test(self):
        self.plot()
        return super(NESReconstruct, self)._test()
