...
from src.models.nes8 import NeuralEvolutionarySpectra
...
class NESForecast(ForecastExp, NESParameters):
    ...
    def _init_model(self):
        ...
        A_init, omegas  = get_initial_amplitude_stft_torch(
            torch.tensor(self.dataloader.test_dataset.scaled_data.squeeze()), 
            n_fft=self.windows, 
            hop_length=self.windows,
        )
        ...
        # initialize A in training time 
        S_train_init, omega, R_est = get_initial_spectrum_benowitz_targetM(
            self.dataloader.train_dataset.scaled_data.squeeze(), 
            M_target=self.M,       # consider lags
        )
        train_mag = torch.tensor(np.sqrt(S_train_init)).float() 
        train_phase = torch.rand_like(train_mag) * (2 * np.pi) - np.pi
        train_A = torch.polar(train_mag, train_phase)  # Bt, M

        # A in testing time to avoid future lable leak
        S_test_init, omega, R_est = get_initial_spectrum_benowitz_targetM(
            self.dataloader.test_dataset.scaled_data.squeeze(), 
            M_target=self.M,       # consider lags
        )
        test_mag = torch.tensor(np.sqrt(S_test_init)).float() 
        test_phase = torch.rand_like(test_mag) * (2 * np.pi) - np.pi
        test_A = torch.polar(test_mag, test_phase) # Bt, M
        A = torch.concat([train_A, test_A], dim=0)
        self.model = NeuralEvolutionarySpectra(
            self.dataset.length, 
            self.M,
            self.device,
